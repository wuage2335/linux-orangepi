...
- [x] RTSP shared server、客户端断开重连和关键帧恢复。
- [x] 修复30.05fps实采与固定30fps PTS造成的累计漂移；改用单调实时时钟。
- [x] GStreamer两分钟稳定播放；五组延迟60/70/10/160/60ms，平均72ms。
- [x] GStreamer与VLC断开重连通过；VLC稳定但约400ms，仅作兼容性客户端。
- [x] 阶段4/5总回归、PM和CSI/ISP/MPP/IOMMU fault检查通过。

完成标准：PC 端稳定播放，端到端延迟可测量，链路断开后能够恢复。

阶段5验收记录（2026-08-28）：推荐基线为1080p30、H.264 CBR 8Mbps、GOP30、
queue2、MTU1200、DMA-BUF和GStreamer 30ms接收缓存。手机播放由用户明确降为
非重点可选项，不作为阶段5关闭条件。详细证据见
`stage5_rtp_streaming_validation.md`和`stage5_rtsp_recovery_validation.md`。
...
三种物理场景的最终画质已通过。详细证据见`stage6_rkaiq_3a_validation.md`。
