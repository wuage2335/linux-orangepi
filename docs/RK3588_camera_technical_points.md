...
当前执行约定（2026-08-06）：学习驱动的 2A 静态脚手架已经开始编写，包括 controls/PM 所需的状态与常量，并移除了会与正式驱动竞争的 `i2c:ovti,ov13850` alias。用户要求暂缓编译和实机验证，待阶段 2 编写完成后统一进行；因此这里的“下一步编译验证”是后续验证门，而不是当前动作。
